...
- [ ] 1.1 In the "Review" tab, render one file with `FileDiff` from the shimmed `@pierre/diffs/react` and one line annotation with a React button; verify in the running app that the code theme matches bb's diff panel and the button is clickable, and record the result (D4 or fallback) in design.md
...
- [ ] 2.1 Extract the PR lookup from `getInsight` in `server.ts` into a `resolvePr(threadId)` function used by both features; verify the existing tests still pass with `npm test`
- [ ] 2.2 Implement `parsePrFiles` (path, status, patch or null) and verify tests: a normal file, a renamed file, and a file without a patch
